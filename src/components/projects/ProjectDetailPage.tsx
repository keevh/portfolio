...
export function ProjectDetailPage({ project }: Props) {
  ...
  const proseClass = 'font-body-md text-base leading-7 text-on-surface-variant sm:text-lg';
  const sectionHeadingClass = 'font-headline-md text-2xl text-white sm:text-3xl';
  const eyebrowClass = 'font-label-caps text-[11px] tracking-[0.18em] text-primary-container';
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface">
      <article className="relative mx-auto max-w-6xl px-4 pt-28 pb-24 sm:px-6 sm:pt-32 lg:px-8">
        {/* §1 Hero split */}
        <section>
          <a
            href={getHomeHref()}
            onClick={() => setPendingHomeScrollTarget('projects')}
            className="mb-10 inline-flex items-center gap-2 font-code-sm text-on-surface-variant transition-colors hover:text-white"
          >
            <ArrowLeft size={16} />
            {copy.back}
          </a>

          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,520px)] lg:items-center lg:gap-12">
            <div>
              {tags.length > 0 && (
                <div className="mb-6 flex flex-wrap gap-3">
                  {tags.map((tag, index) => (
                    <span
                      key={`${tag.label}-${index}`}
                      className="inline-flex items-center gap-2 rounded-full border border-white/8 bg-white/[0.04] px-3 py-1.5 font-code-sm text-sm text-gray-200"
                    >
              ...
              <h1 className="font-headline-lg text-4xl text-white sm:text-5xl lg:text-6xl">{content.title}</h1>
              <p className="mt-5 max-w-xl font-body-md text-lg leading-relaxed text-on-surface-variant sm:text-xl">
                {detail.tagline}
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {links.map((link) => {
                  ...
                  const buttonClass = link.type === 'demo'
                    ? 'bg-primary-container text-on-primary-container hover:bg-primary glow-primary-hover'
                ...
                {project.liveUrl && !links.some((link) => link.url === project.liveUrl) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-primary-container px-5 font-label-caps text-xs text-on-primary-container transition-all hover:bg-primary glow-primary-hover"
                  >
            ...
            <div className="relative">
              <div className="pointer-events-none absolute -inset-6 -z-10 rounded-[40px] bg-primary-container/10 blur-[80px]" />
              <div className="relative aspect-[4/3] overflow-hidden rounded-[28px] border border-white/8 bg-surface/60 shadow-[0_20px_50px_rgba(0,0,0,0.35)]">
                <img
          ...
          {metaItems.length > 0 && (
            <dl className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-[20px] border border-white/8 bg-white/8 sm:grid-cols-4">
              {metaItems.map((item) => (
                <div key={item.label} className="bg-surface/60 p-4 sm:p-5">
                  <dt className={`${eyebrowClass} text-on-surface-variant`}>{item.label}</dt>
        ...
        <div className="mt-16 space-y-16 sm:mt-20 sm:space-y-20">
          ...
          {images.length > 1 && (
            ...
            >
              <h2 className={`mb-6 ${sectionHeadingClass}`}>{copy.gallery}</h2>
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-stretch">
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden rounded-[28px] border border-white/8 bg-white/[0.02] sm:min-h-[420px] xl:h-[520px] xl:min-h-0 xl:aspect-auto">
                    <motion.button
                      type="button"
                ...
                <div
                  ...
                  onMouseEnter={() => setIsGalleryHovered(true)}
                  onMouseLeave={() => setIsGalleryHovered(false)}
                  className="flex gap-[10px] overflow-x-auto pb-2 sm:hidden scroll-smooth [&::-webkit-scrollbar]:h-4 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20"
                >
                  {galleryItems.map((item, index) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.96 }}
                        className={`relative h-20 w-32 flex-none snap-start overflow-hidden rounded-xl border-2 transition-all ${
                          isActive
                            ? 'border-primary-container shadow-[0_0_18px_rgba(52,211,153,0.25)]'
                  ...
                  {galleryItems.map((item, index) => (
                    <motion.button
                      ...
                      className={`group relative overflow-hidden rounded-[24px] border bg-white/[0.02] transition-all xl:min-h-[160px] ${
                        activeGalleryIndex === index ? 'border-primary-container shadow-[0_0_20px_rgba(52,211,153,0.2)]' : 'border-white/8 hover:border-white/20'
                      }`}
                    >
                      <img
                        src={item.src}
          ...
          {/* §5 Core coverage (bento) + credentials */}
            ...
            <section className={`grid gap-8 ${hasCredentials ? 'xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start' : ''}`}>
              ...
              {hasCredentials && (
                <aside className="space-y-4 rounded-[28px] border border-white/8 bg-white/[0.02] p-6 sm:p-8">
                  <h2 className="font-headline-md text-xl text-white">{copy.credentials}</h2>
                  <div className="space-y-3">
                    {credentials.map((item) => (
                      <div key={`${item.label}-${item.value}`} className="rounded-[22px] border border-white/8 bg-background/50 p-4">
                        <p className={`${eyebrowClass} text-on-surface-variant`}>{item.label}</p>
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {project.stack.map((item) => {
                  ...
                  return (
                    <div
                      key={item}
                      className="inline-flex items-center gap-2.5 rounded-full border border-white/8 bg-white/[0.03] px-4 py-2 transition-colors hover:border-primary-container/40"
                    >
                      {visual.logo && (
                        <span className="flex h-6 w-6 items-center justify-center rounded-md bg-white/95">
                          <img src={visual.logo} alt={visual.label} className="h-5 w-5 object-contain" loading="lazy" />
                        </span>
                      )}
                      <span className="font-code-sm text-sm text-on-surface">{item}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* §5 Architecture & domain (zig-zag diagrams) */}
          {diagrams.length > 0 && (
            <section className="space-y-10 border-t border-white/8 pt-12">
              <div className="max-w-3xl space-y-3">
                <h2 className={sectionHeadingClass}>{copy.diagrams}</h2>
                <p className={proseClass}>{copy.diagramsIntro}</p>
              </div>
                    ...
                    >
                      ...
                      <div className={`relative overflow-hidden rounded-[28px] border border-white/8 bg-surface/60 p-2 sm:p-4 ${isReversed ? 'lg:order-1' : ''}`}>
                        ...
                        <img
                          ...
                          alt={diagram.title}
                          loading="lazy"
                          className="mx-auto max-h-[420px] w-full object-contain lg:max-h-none"
                        />
                      </div>
                    </motion.article>
                  );
