...
% Compilar: cd /home/keevh/Documentos/dev/portfolio/public/cv && lualatex -interaction=nonstopmode Kevin-Gallardo-CV.tex && lualatex -interaction=nonstopmode Kevin-Gallardo-CV.tex
%-------------------------

\documentclass[legalpaper,11pt]{article}

\usepackage[
  top=1.1cm, bottom=1.1cm, left=1.9cm, right=1.9cm
]{geometry}
...
% --- Listas -------------------------------------------------------------
\setlist[itemize]{
  leftmargin=1.1em, itemsep=1.5pt, topsep=2pt, parsep=0pt,
...
% Pildora de stack
\newcommand{\stack}[1]{{\color{accentdark}\small\itshape #1}\par\vspace{2pt}}
% Item de la barra de habilidades
\newcommand{\skillrow}[2]{%
  \noindent\begin{tabularx}{\textwidth}{@{}p{3.1cm}@{}X@{}}
    \textbf{\color{ink}#1} & #2
  \end{tabularx}\par\vspace{2.5pt}
}
% Curso en una sola línea: titulo + institución gris, año a la derecha.
% Se usa tabular* (no tabularx) porque la columna X hace expansión de prueba
% que rompe con cambios de fuente/color en la misma celda.
% #4 = URL del certificado: el título se vuelve enlace (accentdark) y se añade
% un ícono de premio como pista de que abre algo.
\newcommand{\course}[4]{%
  \noindent\begin{tabular*}{\textwidth}{@{}l@{\extracolsep{\fill}}r@{}}
    \href{#4}{\color{accentdark}\textbf{#1}}\,{\color{muted}\small$\cdot$\,#2}\,%
      \href{#4}{\color{accent}\footnotesize\faAward} & {\color{muted}\small #3}
  \end{tabular*}\par\vspace{3pt}
}
% Foto de perfil circular para el encabezado.
\newcommand{\profilephoto}{%
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \clip (0,0) circle (1cm);
    \node at (0,0) {\includegraphics[width=2cm,height=2cm]{Kevin-Gallardo-photo.png}};
    \draw[accent, line width=0.8pt] (0,0) circle (1cm);
  \end{tikzpicture}%
}
...
{\color{accent}\large Desarrollador Web Junior\, \textcolor{muted}{$|$}\, JavaScript \textcolor{muted}{$|$} React \textcolor{muted}{$|$} Node.js}
\end{minipage}\hfill
\begin{minipage}[c]{0.20\textwidth}
\centering\profilephoto
\end{minipage}\\[1pt]
{\color{rule}\rule{\textwidth}{1pt}}\\[6pt]
{\small
\faGlobe~Portafolio: \href{https://keevh.dev/}{keevh.dev}\quad
...
\faWhatsapp~\href{https://wa.me/573172172768?text=Hola\%20he\%20visto\%20tu\%20CV\%20y\%20me\%20gustar\%C3\%ADa\%20hablar\%20contigo\%20sobre\%20una\%20oportunidad\%20laboral.}{3172172768}
}
...
\small
\begin{sloppypar}
Desarrollador web junior, estudiante de \textbf{Ingeniería de Sistemas} (último semestre), con disponibilidad inmediata para trabajar a tiempo completo. Cuento con experiencia práctica construyendo proyectos full stack con \textbf{JavaScript,
...
\skillrow{Complementario}{Python, fundamentos de Java y C\#, IoT, MQTT, IA aplicada, Excel}
...
\entry{Trimly~\textcolor{muted}{\small— Acortador de URLs open source y autoalojable}}{\href{https://trimly.keevh.dev}{\color{accentdark}\small trimly.keevh.dev}}
...
\entry{Bakery~\textcolor{muted}{\small— E-commerce de panadería full-stack}}{\href{https://bakery.keevh.dev}{\color{accentdark}\small bakery.keevh.dev}}
\stack{Next.js 16 · React 19 · TypeScript · Tailwind CSS · Drizzle ORM · PostgreSQL · Zustand}
\begin{itemize}
  \small
  \item Tienda en línea con catálogo de productos y panel administrativo para gestionar el negocio.
  \item API REST propia para productos y pedidos sobre \textbf{PostgreSQL} con \textbf{Drizzle ORM} y manejo de estado con Zustand.
  \item Arquitectura modular por capas (infraestructura y presentación) sobre Next.js App Router, con optimización SEO (sitemap y robots).
\end{itemize}
\vspace{2.5pt}
...
\subentry{Proyectos personales y desarrollo web independiente — Remoto}
\begin{itemize}
  \small
...
\subentry{Ingeniería de Sistemas — último semestre · Bucaramanga, Colombia}
\subentry{Disponibilidad inmediata para trabajar a tiempo completo}

% ============ CURSOS ============
\section{Cursos y Certificaciones}
\course{Flutter — Móvil: De cero a experto}{DevTalles $\cdot$ Fernando Herrera $\cdot$ 50 h}{2026}{https://keevh.dev/cv/certificados/curso_flutter.pdf}
\course{C\# TOTAL — Programador Experto en 28 días}{Udemy $\cdot$ Denis Panjuta $\cdot$ 37 h}{2026}{https://keevh.dev/cv/certificados/curso_csharp.pdf}
\course{Nest: Desarrollo backend escalable con Node}{Udemy $\cdot$ DevTalles $\cdot$ Fernando Herrera $\cdot$ 24 h}{2026}{https://keevh.dev/cv/certificados/curso_nest.pdf}
